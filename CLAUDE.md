...
├── fixtures/          # Playwright fixtures (index.js exports extended test)
├── tests/             # Playwright spec files only
├── .claude/
...
## Testing Conventions
- Spec files go in `tests/` as `<feature-name>.spec.js`
- Page Object Models go in `pages/` (root-level, not inside tests/)
- Fixtures go in `fixtures/` (root-level); import `test` from `../fixtures/index.js`
...
- No `page.waitForTimeout()` — use `locator.waitFor()` in POMs, `expect().toBeVisible()` in tests
- Assertions belong in spec files, not in page objects
- Use `cleanSession` fixture for any test that needs login + cleared bookings state
