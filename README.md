...
- **Node.js 22.5+** (required by Playwright 1.48+ which uses the built-in `node:sqlite`)
- **MySQL 8+** running locally (or a remote instance)
- **pnpm 9+** — install via `npm i -g pnpm` or `corepack enable`
- **Docker** (optional) — only required to run tests in a container
...
| `pnpm lint` | Run ESLint on the frontend (Next.js) |
...
```json
{
  ...
  "customerEmail": "yourname@example.com",
...
```
eventhub/
├── package.json              ← Root scripts (dev, setup, seed, test, test:docker)
├── Dockerfile                ← Container image for running E2E tests with pnpm
├── playwright.config.ts      ← Playwright config (baseURL, Chromium only)
├── README.md
│
...
├── fixtures/
│   └── index.js              ← Extended test + cleanSession fixture
...
Tests run against the live staging site at `https://eventhub.rahulshettyacademy.com` using **Playwright** (Chromium only).
...
npx playwright test tests/booking-management.spec.js --reporter=line

# UI mode (visual debugger)
pnpm test:ui

# inside Docker with pnpm (no local browser install needed)
pnpm test:docker
```
...
├── LoginPage.js                → goto(), login(email, password)
├── EventsPage.js               → goto(), firstAvailableCard()
├── EventDetailPage.js          → fillAndSubmit(), getBookingRef(), viewMyBookingsLink
├── BookingsPage.js             → goto(), cardByRef(), clearAll()
└── BookingDetailPage.js        → cancelBooking(), section locators

fixtures/
└── index.js                    ← Extended test + cleanSession composite fixture
...
Each page is a plain class. Locators are defined in the constructor; methods represent user actions. Assertions stay in the spec files.
...
`tests/fixtures/index.js` exports an extended `test` with two layers of fixtures:

**Individual page fixtures** — inject a single POM, useful when you need custom setup:

```javascript
test('example', async ({ eventsPage, bookingsPage }) => { … });
```

**`cleanSession` composite fixture** — logs in as the test user, clears all prior bookings, and yields every POM ready to use. All tests in `booking-management.spec.js` use this:
...
The `Dockerfile` builds a self-contained test runner using pnpm:

- Base image: `node:22-slim`
- pnpm installed via `corepack`
- `pnpm import` converts `package-lock.json` → `pnpm-lock.yaml` automatically on first build
- Chromium + all OS dependencies installed via `playwright install --with-deps chromium`
...
**Example Playwright usage (preferred locator style):**
```javascript
